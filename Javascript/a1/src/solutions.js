...
function normalizeVariable(value) {
  ...
  result = result.trim();  // removes whitespace from both ends of a string and returns a new string, without modifying the original string
  result = result.replace(/[^A-Z]+/g, '_'); // select all except capital letters and replace them with '_'
...
function createIframe(src, height, allowFullScreen) {
  ...
  if(allowFullScreen){
      ...
      result = '<iframe src="' + s + '" height="' + h + '" allowfullscreen></iframe>';  
  }
  else{
      s = src.trim();
      h = parseInt(height);
      ...
      if(h == NaN){
        ...
        result = '<iframe src="' + s + '"></iframe>';
      }
      else{
        result = '<iframe src="' + s + '" height="' + h + '"></iframe>';
      }
  }
...
function fixPostalCode(postalCode) {
  ...
  const regex = /([ABCEGHJKLMNPRSTVXY]\d[ABCEGHJKLMNPRSTVWXYZ][ ]?\d[ABCEGHJKLMNPRSTVWXYZ]\d)/g;

  result = postalCode.toUpperCase();
  result = result.trim();

  found = result.match(regex);

  if(found != null){
    
    found = found.toString();
  
    if(result.length > 7){ //too many char
      throw new Error; 
    }
    else if (found.charAt(3) !== ' ') { //add space
      result =  postalCode.slice(0, 3) + ' ' + postalCode.slice(3, 6);
      //console.log("........"+result);
    } 
    else if(found != null){ //matched with regex
        result = found
        //console.log("/////"+result); 
...
function toProvince(postalCode, useShortForm) {
  ...
  code = fixPostalCode(postalCode);
  //what value return: throw new Error ? and How to catch it ?
  code = code.toString();
  code = code.toUpperCase();

  if ((code.charAt(0) == 'K') || (code.charAt(0) == 'L') || (code.charAt(0) == 'M') || (code.charAt(0) == 'N') || (code.charAt(0) == 'P')) {
    code = 'Ontario';
  ...
  else if((code.charAt(0) == 'G') || (code.charAt(0) == 'H') || (code.charAt(0) == 'J')){
    code = 'Quebec';
    ...
    code = 'Northwest Territories and Nunavut';
    if(useShortForm){
      code = 'NT';
    }
  }
...
function normalizeCoord(value) {
  ...
  result = value.toString().match(regex);
